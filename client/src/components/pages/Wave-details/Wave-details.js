...
class WaveDetails extends Component {
    ...
    removeFavourite() {
        const waveid = this.props.match.params.id
        this.authService
            .removeFavourites(waveid, this.props.loggedUser)
            .then(() => this.setState({ added: false }, () => this.loadWaves()))
            .catch(err => console.log(err))
    }
                        ...
                        <Container className={this.state.move ? 'active' : 'not-active'} as="section" style={{ marginTop: 100, marginBottom: 100 }}>
                            ...
                            {this.state.wave
                                ...
                                <>
                                    <Row>
                                        <Col md={{ span: 6 }}>
                                            ...
                                            <ButtonGroup size="mb" style={{ marginBottom: 20 }}>
                                                ...
                                                {this.props.loggedUser?.favourites.includes(this.state.wave?._id) ?
                                                    <Button variant="dark" onClick={() => this.removeFavourite()} > Remove from favourites</Button>
                                                    :
                                    ...
                                    <hr></hr>
                                    <Row>
                                        <Col><Comments wave_id={this.state.wave._id} commentUser={this.props.loggedUser?._id} handleAlert={this.props.handleAlert}></Comments></Col>
                                    </Row>
                                    ...
                                    <Modal show={this.state.showForm} onHide={() => this.togglemodalForm(false)} size='xl'>
                                        ...
                                        <Modal.Body>
                                            <WaveForm closeModal={() => this.togglemodalForm(false)} wave={this.state?.wave} modalType="Edit" refreshList={() => this.loadWaves()} handleAlert={this.props.handleAlert} />
                                        </Modal.Body>
                                    </Modal>
                                </>
                                :
